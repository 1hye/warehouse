...
`harness-collab/` 是 Spring Boot Harness 模板的 AI 协作文档中心，存放整个研发生命周期（Dev_Lifecycle）所需的规范文档、模板和方法论指南。Cursor Agent 在每次对话中通过 rules 文件自动感知这些文档的存在，并在适当时机引导开发者创建和维护对应文档。
...
`harness-collab/` 中的文档与 `.cursor/rules/` 中的 rules 文件协同工作。当文档内容发生重大变更时，需要同步更新对应规则文件：

| harness-collab 文档 | 对应 steering 文件 | 同步触发条件 |
|--------------------|--------------------|-------------|
| `05-methodology/architecture-constraints.md` | `.cursor/rules/java-engineering-standards.mdc` | 架构规则变更时 |
| `05-methodology/dev-workflow.md` | `.cursor/rules/project-lifecycle.mdc` | 流程阶段变更时 |
| `AGENTS.md` | `.cursor/rules/ai-collaboration-protocol.mdc` | 协作协议变更时 |
| `04-api-docs/` 下任意文档 | `.cursor/rules/api-doc-sync-protocol.mdc` | API 文档规范变更时 |

**同步步骤**：
1. 更新 `harness-collab/` 中的对应文档
2. 检查 `.cursor/rules/` 中对应规则文件的约束是否需要同步调整
