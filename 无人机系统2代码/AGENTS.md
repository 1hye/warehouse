...
本文件是 Spring Boot Harness 工作区的 AI 协作协议入口。完整协议定义在 [harness-collab/AGENTS.md](harness-collab/AGENTS.md)，所有在本工作区内工作的 Cursor Agent 必须遵守该协议。

---

## Harness 监管机制

Harness 通过两种 Cursor 原生机制对工作区内的所有子项目实施自动监管：

**Steering 文件自动加载**

`.cursor/rules/` 目录下的规则文件由 Cursor 自动加载。无论开发者在工作区的哪个子目录下工作，以下约束始终生效：

- `java-engineering-standards.md` — 四层架构约束、编码标准
- `testing-quality-standards.md` — 测试分层策略、覆盖率要求
- `api-doc-sync-protocol.md` — API 文档同步规则
- `ai-collaboration-protocol.md` — Cursor Agent 行为规范、交付标准
- `project-lifecycle.md` — Dev_Lifecycle 六阶段流程及门禁

**Hooks 自动触发**

`.cursor/hooks.json` 和 `.cursor/hooks/` 下的 Hook 在特定 IDE 事件发生时自动触发，无需手动调用：
...
## Cursor Agent 核心行为约束（快速参考）

在本工作区内，Cursor Agent 必须遵守以下五条核心规则：
