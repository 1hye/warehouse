...
> **核心定位**：Harness 是部署在工作区根目录的"监管层"。当你在同一工作区内创建任意 Spring Boot 子项目时，Harness 的 rules、规范文档和验证脚本**自动对该子项目生效**，AI Agent 在整个研发生命周期中均受 Harness 约束，无需额外配置。

---

## 模板概述

Spring Boot Harness 工程管理模板通过 Trae IDE 的原生机制（rules、规范文档）将工程标准内嵌到 AI 辅助开发流程中，实现**规范即代码、门禁即流程**的研发基线。
...
│       ├── ai-collaboration-protocol.md     # AI 协作协议（AI Agent 行为约束）
...
**步骤 3：在 Trae IDE 中打开工作区根目录**

Trae IDE 自动加载 `.trae/rules/` 下所有规则文件，约束立即生效，无需任何额外配置。

**步骤 4：验证规则文件已加载**

```powershell
# 确认 5 个规则文件已存在
(Get-ChildItem ".trae/rules/*.md").Count
# 预期输出：5
```

**步骤 5：开始开发**

向 AI Agent 发起任何开发请求，Agent 会自动执行 GATE-01 检查（确认需求文档是否存在），引导你走完整的 Dev_Lifecycle 六阶段流程。
...
AI Agent 在本工作区内遵循 Dev_Lifecycle 六阶段流程：
