...
本手册定义 AI Agent 在 Spring Boot Harness 工作区中的操作规范，包括每个研发阶段的具体行为、交付摘要格式和常用命令速查表。
...
1. **接收需求描述**：当开发者描述新功能时，Agent 首先确认是否已有对应的需求文档
   ```
...
每次完成一个功能或阶段后，Agent 必须输出以下格式的交付摘要：
...
与 Agent 协作时，提供充分的上下文信息能显著提升代码质量：
...
Trae IDE 的 `.trae/rules/` 规则文件会在对话中自动生效，Agent 会自动遵循以下约束：
- 代码生成前检查需求和设计文档是否存在
- 新建 Java 类时自动检查包路径分层合法性
- 业务代码变更后提示同步更新测试文件和 API 文档
