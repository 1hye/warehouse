...
# ── 1. Trae Rules（5个，当前 IDE 主用）────────────────
echo -e "${YELLOW}[1/7] Trae Rules 文件体系${NC}"
check_file ".trae/rules/java-engineering-standards.md"
check_file ".trae/rules/testing-quality-standards.md"
check_file ".trae/rules/api-doc-sync-protocol.md"
check_file ".trae/rules/ai-collaboration-protocol.md"
check_file ".trae/rules/project-lifecycle.md"
echo ""

# ── 2. Cursor Rules（5个，历史兼容）───────────────────
echo -e "${YELLOW}[2/7] Cursor Rules 文件体系（历史兼容）${NC}"
check_file ".cursor/rules/java-engineering-standards.mdc"
check_file ".cursor/rules/testing-quality-standards.mdc"
check_file ".cursor/rules/api-doc-sync-protocol.mdc"
check_file ".cursor/rules/ai-collaboration-protocol.mdc"
check_file ".cursor/rules/project-lifecycle.mdc"
echo ""
...
# ── 7. Rules Frontmatter 验证──────────────────────────
echo -e "${YELLOW}[7/7] Rules Frontmatter 验证（alwaysApply: true）${NC}"
RULES_COUNT=0
...
for f in .trae/rules/*.md; do
  if [ -f "$f" ]; then
    if grep -q "alwaysApply: true" "$f"; then
      echo -e "${GREEN}✅ $f 包含 alwaysApply: true${NC}"
      RULES_COUNT=$((RULES_COUNT + 1))
    else
      echo -e "${RED}❌ $f 缺少 alwaysApply: true 元数据${NC}"
      ERRORS=$((ERRORS + 1))
    fi
    CHECKS=$((CHECKS + 1))
  fi
...
if [ $ERRORS -eq 0 ]; then
  ...
  echo -e "${GREEN}   Rules 文件数量：$RULES_COUNT（均包含 alwaysApply: true）${NC}"
  echo ""
  echo "Harness 模板已就绪，可直接使用。"
  echo "新建 Spring Boot 子项目后，在 Trae IDE 中打开工作区根目录即可开始受监管的开发。"
else
